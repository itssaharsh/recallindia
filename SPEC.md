...
|---|---|---|---|---|
| cdsco_nsq · adapter `cdsco_portal` | CDSCO NSQ portal (cdscoonline.gov.in DataTables, live since Aug 2025) | GET JSON, no auth, no cookie (a browser User-Agent is enough; verified live 2026-09-18 by the deployed poller): `/CDSCO/reportingYears?tab=nsq` → `/CDSCO/publicReportingMonths?year=YYYY&tab=nsq` → `/CDSCO/filteredNsqDrugTable?month=MON-YYYY&source=All&tab=nsq` (`publicNsqDrugTable` = latest month). Rows in `aaData[]`: `str_product_name, str_batch_no, dt_manufacturing_date, dt_expiry_date, str_manufactured_by, str_nsq_result, str_reporting_source, str_reported_by_lab_or_state, dt_reporting_month_year`. | daily (month param = newest month, re-pull current month) | LIVE adapter, 2019→now. Fixture `fixtures/cdsco/nsq_jul2026_all.json` (239 rows). `row_ref = {month, row}`. |
| cdsco_nsq · adapter `cdsco_pdf` | CDSCO monthly NSQ alert PDFs (archive 2010–Jun 2025) | Listing `download_file_division.jsp?num_id=<b64>` → iframe `src` → PDF from cdsco.gov.in → S3 → **Textract TABLES (primary)** → **pdfplumber `extract_tables` fallback** (proven in P00: clean 8-column table, 59 rows on `nsq_latest.pdf`) → header-keyword normalise (a Bedrock column mapper exists behind `BEDROCK_ENABLED`, default false) | one-off backfill; on demand for `/ingest` | HERO source for the `/ingest` shot. Fixture `fixtures/cdsco/nsq_latest.pdf` (June 2025, 6 pages). Cache PDFs in S3. `row_ref = {page, row}`. Textract works since the account moved to the Paid plan (verified live 2026-09-19); the pdfplumber fallback stays first-class (old archive layouts return <5 Textract rows). |
...
- `notices` — pk `source#notice_id`, attrs: source, adapter? (`cdsco_portal`|`cdsco_pdf`|null), title, product, brand, model, batches[] (or serial_ranges[] / vehicles[{make,model,year_from,year_to}]), hazard_or_failed_test, remedy, published_at, url, raw_excerpt (≤4 KB), pdf_s3_key?, row_ref? (`{page,row}` for PDF, `{month,row}` for portal), mfg_date?, exp_date?, lab?, first_seen_at, updated_at (poller bookkeeping). GSI `brand_lc` → for candidate lookup; `brand_lc = brand_key(brand)` (`common/brands.py`: lower-case, punctuation and legal suffixes dropped, so "Finecure Pharmaceuticals" and "Finecure Pharmaceuticals Ltd." share one key; the display `brand` is kept as the source wrote it). Poller health lives in the same table as `pk=meta#<source>` rows (`last_success_at`, `last_error`, `degraded`, `last_counts`); the feed and API exclude them.
...
**No LLM in the decision path (by design; Bedrock quotas held at 0 on this account, increase denied — model path implemented behind a flag).** Every step below is deterministic Python; `verifier: "deterministic"` is recorded on each case.
1. **Candidates** (Lambda): for an item, query `notices` by brand_lc + fuzzy on model/product (rapidfuzz ≥ 80). Output ≤5 candidates.
2. **Verify** (Lambda, deterministic — the primary path): `covers_item = brand matches (normalised) AND (product/model fuzzy ≥ 90 OR a listed batch/serial/model token appears on the item)`. `quoted_sentence` is the notice's own row text (product + batch + failed test) taken verbatim from `raw_excerpt` — the substring rule is enforced in code, else `covers_item = null`. `reasoning` is a complete human-readable account: `brand 'Ipca' matches; product 'Paracetamol 650' fuzzy 96; batch DL-4471 in listed [DL-4471, DL-4468]; quoted: '<row text>'`. Returns `{covers_item, quoted_sentence, confidence, reasoning, verifier: "deterministic"}`. A Bedrock verifier (`MODEL_VERIFY`, temperature 0, strict JSON, same quote guard; Bedrock error → `covers_item: null`) is implemented behind `BEDROCK_ENABLED` (default false) and is never on the live path.
...
4. **Decide** (Lambda `decide.py`, deterministic; a Choice state cannot pick the best of the Map's list, the Choice after it only routes): `alert` if covers_item && inside; `hold` if covers_item && inside==null, or covers_item==null; `dismiss` otherwise with the reason built from the failing check (`batch DL-4472 not in listed batches [DL-4471, DL-4468]` / `model X not covered; notice lists Y` / `year 2021 outside 2017–2019`). No candidates at all → `clear`: an item state (`no match in N sources as of <time>`), not a case.
5. **Notify** (Lambda): write case, set item.status, SES email to `NOTIFY_EMAIL` (sandbox-verified), in-app event.
6. **WaitForApproval** (`lambda:invoke.waitForTaskToken`, alert decisions only): the Lambda stores the task token and `approval.token_issued_at` on the case (the API never returns the token). `POST /cases/{id}/approve` → `SendTaskSuccess`; `POST /cases/{id}/reject` → `SendTaskFailure(error="Rejected")`, case `rejected`. Single-use: a conditional update removes the token, a second answer is 409; an unknown case is 404, a non-alert case 409, and neither reaches Step Functions. No heartbeat; `TimeoutSeconds` 86400 → ExpireApproval, case `expired`.
7. **Claim** (Lambda → Jinja template → reportlab PDF; no model): letter to seller/pharmacy/dealer citing notice id, batch, remedy; if `purchase_date > published_at` add Consumer Protection Act paragraph ("sold after notice"). PDF → claims bucket; `GET /cases/{id}/claim` returns a 10-minute presigned link; the plain text stays on the case (`claim_text`).
8. **Evidence** (Lambda): snapshot the source's own bytes (CDSCO archive: the PDF in the raw bucket; CDSCO portal: the notice's row of the month's portal JSON; NHTSA / CPSC / openFDA: the recall record from their API; unreachable → the stored notice, labelled `stored_notice`) → evidence bucket with Object Lock (GOVERNANCE, retain 30 days, set on the PUT) → SHA-256 → KMS `Sign` (RSA_2048, RSASSA_PKCS1_V1_5_SHA_256, digest) → `{sha256, kms_key_id, signature_b64, object_lock_retain_until, snapshot_s3_key, snapshot_version_id, signed_at}` on the case (ADR-007). `GET /cases/{id}/verify-evidence[?tamper=1]` re-reads that object version, re-hashes it (`tamper=1` flips one byte in memory, a labelled demo control) and calls KMS `Verify` → `{valid, sha256, signed_at, key_id}`.
...
App API: `POST /items` (paste lines | strip photo → Textract batch OCR | vehicle reg) · `POST /items/{id}/check` (starts state machine) · `GET /cases/{id}` · `POST /cases/{id}/approve|reject` · `GET /cases/{id}/claim` · `GET /cases/{id}/verify-evidence[?tamper=1]` · `POST /ingest/run` (demo trigger).
