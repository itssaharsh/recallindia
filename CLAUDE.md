...
- Rules decide. **No LLM in the decision path (by design; Bedrock quotas held at 0 on this account, increase denied — model path implemented behind a flag).** Candidates, verify, range check, decide, normalise and the claim letter are deterministic Python / templates; the Bedrock verifier and normaliser stay implemented behind `BEDROCK_ENABLED` (default false) and are never attempted in live acceptance.
...
- Bedrock is unavailable for the whole hackathon (quotas held at 0, increase denied): `BEDROCK_ENABLED` defaults to false everywhere (`.env.example`, `template.yaml`), every call then raises `BedrockUnavailable` and the deterministic path runs. Other AWS AI services in use (wrappers in `backend/common/aws_ai.py`, each with a DEMO_MODE fixture): Comprehend `DetectEntities` (paste-import, P06), Translate en→hi + Polly `Kajal`/`Aditi` (voice note, P10).
