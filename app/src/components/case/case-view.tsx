...
function Field({
  ...
  return (
    <div className="grid grid-cols-[7rem_minmax(0,1fr)] gap-3 border-b border-line py-2 text-[13px]">
      ...
      <dd
        className={`min-w-0 text-ink ${mono ? "font-mono text-[12.5px]" : ""}`}
      >
        {children}
      </dd>
    </div>
...
export function CaseView() {
    ...
    (async () => {
      try {
        ...
        const [it, n, st] = await Promise.all([
          apiGet<Item>(`/items/${enc(c.item_id)}`, demo, signal).catch(
            () => null,
          ),
          c.notice_id
            ? apiGet<Notice>(
                `/v1/notices/${enc(c.notice_id)}`,
                demo,
                signal,
              ).catch(() => null)
            : null,
          apiGet<CheckStatus>(
            `/items/${enc(c.item_id)}/check-status`,
            demo,
            signal,
          ).catch(() => null),
        ]);
  ...
  usePoll(
    async (signal) => {
      if (!kase) return;
      const fresh = await apiGet<Case>(
        `/cases/${enc(kase.case_id)}`,
        demo,
        signal,
      );
      ...
      try {
        ...
        if (err instanceof ApiError && err.status === 409) {
          try {
            ...
            setAnswerError(
              `Couldn't reach the approval step (${message}). Try again.`,
            );
          }
        } else {
          setAnswerError(
  ...
  if (loadError || !kase) {
    ...
    return (
      <EmptyState
        ...
        what={
          missing
            ? "That case isn't in this household"
  ...
  const chipCode =
    item?.batch ||
    item?.serial ||
    (item?.kind === "vehicle" && item.year ? String(item.year) : "");
  // the demo household's case is readable by anyone and answerable by nobody
  const readOnly =
    demo || (c.household_id ?? DEMO_HOUSEHOLD) === DEMO_HOUSEHOLD;

  return (
    <article
      aria-labelledby="case-title"
      className="mx-auto max-w-[1200px] px-5 py-5 md:py-7"
    >
      <nav
        aria-label="Case"
      ...
      <header className="space-y-3 border-b border-line pb-6">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
          ...
          {c.created_at && (
            <span className="text-xs text-muted">
              case opened {fmtDay(c.created_at)}
            </span>
          )}
        ...
        {subLine && (
          <p className="max-w-2xl text-[15px] text-muted">{subLine}</p>
        )}
            ...
            {excerpt && (
              <SourceExcerpt
                ...
                caption={
                  notice ? (
                    <>
                      {sourceLabel(notice.source)}&apos;s own words, published{" "}
                      {fmtDay(notice.published_at)}
                      {notice.url && (
                        <>
                          {" · "}
                          <a
              ...
              <dl>
                {notice.source === "cdsco_nsq"
                  ? notice.hazard_or_failed_test && (
                      <Field label="Failed test">
                        {notice.hazard_or_failed_test}
                      </Field>
                    )
                  : riskSentence(notice) && (
                      <Field label="Hazard">{riskSentence(notice)}</Field>
                    )}
                {notice.lab && <Field label="Tested by">{notice.lab}</Field>}
                <Field label="Remedy">
                  {notice.remedy || (
                    <span className="text-muted">
                      Not stated by the source: the letter asks for a refund or
                      a replacement
                    </span>
                  )}
                </Field>
              ...
              <dl className="space-y-1.5 border-t border-line pt-3 text-[12px]">
                ...
                <div className="flex justify-between gap-3">
                  ...
                  <dd className="min-w-0 truncate font-mono text-ink">
                    {notice?.notice_id ?? c.notice_id}
                  </dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-muted">Source</dt>
                  <dd className="text-ink">
                    {notice ? sourceLabel(notice.source) : "—"}
                  </dd>
                </div>
              </dl>
            </div>
