...
import type { Item, NormaliseResult, OcrResult, PasteRow, UploadTicket, OcrWord } from "@/lib/types";

type Created = { items: Item[]; count: number };
const message = (err: unknown) =>
  err instanceof ApiError || err instanceof PhotoError || err instanceof Error ? err.message : String(err);
 ...
 */
function ScanPhoto({ src, words, batch }: { src: string; words: OcrWord[]; batch: string }) {
  ...
  return (
    <figure className="space-y-2">
      <div className="relative overflow-hidden rounded-md border border-line bg-surface-2">
        ...
        <img src={src} alt="The strip you photographed" className="block max-h-64 w-full object-contain" />
        {shown.map((word, i) => (
          <motion.span
            key={`${word.text}-${i}`}
            aria-hidden
            ...
            className={`absolute rounded-[2px] border ${
              word.is_batch ? "border-2 border-primary bg-primary/10" : "border-line-strong/70"
            }`}
            style={{
    ...
    <div className="space-y-4">
      ...
      {ocr && (
        <div className="space-y-3">
          {preview && <ScanPhoto src={preview} words={ocr.words ?? []} batch={form.batch} />}
          <div className="grid gap-3">{field("name", "Product, as printed")}</div>
